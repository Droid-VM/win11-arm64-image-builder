...
# --- 帳號 / 遠端 ---
# 帳號名用 $env:DVM_USERNAME（不是 $env:USERNAME —— 那是 Windows 內建＝目前登入者）；密碼用 $env:DVM_PASSWORD。
$env:DVM_USERNAME = "USER"        # 建立的本機管理員帳號名
$env:DVM_PASSWORD = "DroidVM"     # 密碼（空密碼會擋掉 RDP/SSH 網路登入）
